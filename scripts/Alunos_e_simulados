...
def execute_query():
    ...
    if conn is not None:
        try:
            ...
            SELECT 
                CASE 
                    WHEN q.name LIKE '%LP C1%' THEN 'Língua Portuguesa - Curso 1'
                    WHEN q.name LIKE '%MT C1%' THEN 'Matemática - Curso 1'
                    WHEN q.name LIKE '%LP C2%' THEN 'Língua Portuguesa - Curso 2'
                    WHEN q.name LIKE '%MT C2%' THEN 'Matemática - Curso 2'
                    -- Continue adicionando condições conforme necessário
                    ELSE 'Outro Curso'
                END AS curso,
                ic2.name AS escola,
                ic.name AS turma,
